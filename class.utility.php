...
abstract class AptAdsUtilityDomain extends Gdn_Plugin {
...
class AptAdsUtility {
  ...
  public static function Load($Class){
    ...
    foreach ($Maps As $Map){
      ...
      if(preg_match($Map['Match'],$Class,$Matches)){
        ...
        require_once(PATH_PLUGINS.DS.'AptAds'.($Map['Folder'] ? DS.$Map['Folder']: '').DS.$Map['File']);
        break;
      }
    }
  }
  ...
  public static function InitLoad(){  
    spl_autoload_register('AptAdsUtility::Load');
  }
 
    public function HotLoad($Force =  FALSE) {
        if($Force || C('Plugins.'.$this->Plgn->GetPluginIndex().'.Version')!=$this->Plgn->PluginInfo['Version']){
